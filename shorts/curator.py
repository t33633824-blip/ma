...
Кандидаты с пометкой [история Reddit] это личные истории людей. Для них критерии другие:
неожиданный поворот, узнаваемая бытовая ситуация, можно пересказать за минуту, без насилия,
травм и откровенно взрослых тем. Хорошие истории оценивай наравне с научными темами,
в подборке желательно и то и другое.
...
def format_candidates(cands: list[Candidate], limit: int = 80) -> str:
    ...
    for i, c in enumerate(cands[:limit], 1):
        ...
        label = "история Reddit" if c.kind == "story" else c.source
        lines.append(f"{i}. [{label} | {date}] {c.title}\n   {c.url}\n   {c.summary}")
