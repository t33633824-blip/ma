...
## Отдельный диск для данных

Модели, клипы, фоны и готовые ролики можно держать на другом диске, код остаётся на месте:

```bash
./shorts.sh data move /mnt/gen/shorts-data   # перенесёт уже скачанное и пропишет DATA_DIR в .env
./shorts.sh data show                        # где что лежит и сколько весит
```
